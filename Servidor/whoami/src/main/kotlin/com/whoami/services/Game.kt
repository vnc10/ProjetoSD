package com.whoami.services

import com.whoami.entity.Score
import com.whoami.player.Player
...
class Game(
    ...
    fun savePoints() {
        for (i in this.players) {
            val find = scoreRepository.findById(i.getNickName())
            if (find.isEmpty) {
                val score = Score(i.getNickName(), 0, i.getScore())
                scoreRepository.save(score)
            } else {
                val score = Score(find.get().nome, find.get().totalVitorias, find.get().totalPontos + i.getScore())
                scoreRepository.save(score)
            }
        }
    }
    ...
    fun winner() {
        ...
        if(winner.getScore() == 0){
            sendMessageToAll("NINGUEM PONTUOU NESSA PARTIDA, O JOGO TERMINOU SEM GANHADOR")
        }
        else {
            for (i in this.players){
                if(i.getScore() != winner.getScore()){
            ...
            if(empate){
                sendMessageToAll("A PARTIDA TERMINOU EM EMPATE")
            }
            else {
                val find = scoreRepository.findById(winner.getNickName())
                val score = Score(find.get().nome, find.get().totalVitorias + 1, find.get().totalPontos)
                scoreRepository.save(score)
                sendMessageToAll("O GANHADOR É: " + winner.getNickName())
                winner.messageWin()
            }
        }
        sendMessageToAll("CLASSIFICAÇÃO GERAL DO WHO AM I: ")
        val findAll = scoreRepository.findAllByOrderByTotalVitorias()
        var i = 0
        for (item in findAll) {
            sendMessageToAll(
                "Nick: " + findAll.get(i).nome + " | Total de vitorias: " + findAll.get(i).totalVitorias + " | Total de pontos: " + findAll.get(i).totalPontos
            )
            i++
        }
    }
